...
class Invest(db.Model):
    ...
    def get_filters(self, **kwargs):
        ...
        if 'is_anonymous' in kwargs and kwargs['is_anonymous'] is not None:
            if kwargs['is_anonymous'] is True:
                ...
                filters.append(or_(self.anonymous is None, self.anonymous is False))
        ...
        if 'call' in kwargs and kwargs['call'] is not None:
            if kwargs['call'] is True:
                filters.append(and_(self.call_id is not None, self.call_id != ''))
        ...
        if 'project' in kwargs and kwargs['project'] is not None:
            if kwargs['project'] is True:
                filters.append(and_(self.project_id is not None, self.project_id != ''))
            elif kwargs['project'] is False:
                filters.append(or_(self.project_id is not None, self.project_id == ''))
            else:
    ...
    def donors_list(self, **kwargs):
        ...
        try:
            return db.session.query(self.user_id, User.name, User.id, User.avatar,
                                    func.count(distinct(self.project_id)).label('contributions'),
    ...
    def projects_total(self, **kwargs):
        ...
        try:
            ...
            filters.append(self.status.in_(self.VALID_INVESTS))
            total = db.session.query(func.count(func.distinct(self.project_id))).filter(*filters).scalar()
            if total is None:
                total = 0
            return total
        except MultipleResultsFound:
    ...
    def calls_total(self, **kwargs):
        ...
        try:
            ...
            filters.append(self.status.in_(self.VALID_INVESTS))
            total = db.session.query(func.count(func.distinct(self.call_id))).filter(*filters).scalar()
            if total is None:
                total = 0
            return total
        except MultipleResultsFound:
    ...
    def rewards_per_amount(self, minim=0, maxim=0, **kwargs):
        ...
        filters.append(or_(self.resign is None, self.resign is False, self.resign == 0))
